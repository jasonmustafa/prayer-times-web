...
class App extends React.Component {
  ...
  // Mounts with location data from IP
  componentDidMount() {
    var opts = {
      method: 'GET',
      headers: {}
    };
    fetch('https://geoip-db.com/json/', opts)
      .then(function(getLocation) {
        return getLocation.json();
      })
      .then(geoipLocation => {
        // console.log(geoipLocation);
        const latitude = geoipLocation.latitude;
        const longitude = geoipLocation.longitude;
        ...
        this.setState({
          location: geoipLocation.city + ', ' + geoipLocation.state,
